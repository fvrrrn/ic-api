...
exports.default = (app) => {
    ...
    route.get('/bachelors', async (req, res) => {
        try {
            const request = mssql_1.default.pool1.request();
            const output1 = [];
            const output2 = [];
            const output3 = [];
            let last1 = null;
        ...
        from ic_admission_bachelors_vi_not_passed_yet
            ...
            applicants.push(...tmp);
            const updateTime = `${new Date(Date.now()).getHours()}:${new Date(Date.now()).getMinutes()}:${new Date(Date.now()).getSeconds()}`;
            applicants = applicants.map((v) => (Object.assign(Object.assign({}, v), { update_time: updateTime })));
            res.status(200);
            res.writeHead(200, {
            ...
            logger_1.default.error('admission/applicants error: ', e);
            res.status(401);
        }
    });
    route.get('/masters', async (req, res) => {
        try {
            ...
            if (last3)
                output3.push(last3);
            res.send(output3.map((v) => (Object.assign(Object.assign({}, v), { update_time: `${new Date(Date.now()).getHours()}:${new Date(Date.now()).getMinutes()}:${new Date(Date.now()).getSeconds()}` }))));
        }
        catch (error) {
            logger_1.default.error('admission/sponsorship_types error: ', error);
            res.sendStatus(500);
        }
    });
  ...
  order by concurrency_type_id
    `);
            ...
            logger_1.default.error('UNIVERSITYPROF admission/concurrency_types error: ', err);
            res.sendStatus(500);
        }
    });
    route.get('/spec_types', async (req, res) => {
        try {
            const result = await mssql_1.default.pool1
                .request()
                .query(`SELECT DISTINCT Код id, Наименование name FROM Справочник_Специальности`);
            res.send(result.recordset).status(200);
        }
        catch (error) {
            logger_1.default.error('UNIVERSITYPROF admission/spec_types error: ', error);
            res.sendStatus(500);
        }
    });
    route.get('/sponsorship_types', async (req, res) => {
        try {
            const result = await mssql_1.default.pool1
                .request()
                .query(`SELECT Код id, Наименование name FROM Справочник_ОснованияПоступления`);
            res.send(result.recordset).status(200);
        }
        catch (error) {
            logger_1.default.error('admission/sponsorship_types error: ', error);
    ...
    route.get('/admission_types', async (req, res) => {
        try {
            ...
            logger_1.default.error('admission/admission_types error: ', error);
            res.sendStatus(500);
        }
    });
};
